      ...
      {Object.entries(instances).map(([id, instance]) => (
        <div key={id} className="flex gap-0.5">
          ...
          {React.cloneElement(instance.definition.prototype, {
            onFilterChange: (negate: boolean, operator: FilterOperator, value: any) =>
              handleFilterChange(id, negate, operator, value),
          })}
          <Button
            variant="secondary"
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          {Object.values(definitions).map((it) => (
            <DropdownMenuItem key={it.id} onClick={() => handleAddFilter(it)}>
              {it.label}
            </DropdownMenuItem>
          ))}
        </DropdownMenuContent>
